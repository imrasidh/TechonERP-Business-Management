<?php
/**
 * License API — strict CORS allowlist (no wildcard).
 * Env: TECHON_ALLOWED_ORIGINS, TECHON_CORS_ALLOW_NULL_ORIGIN=1
 */
function techon_license_cors_origin_allowed($origin) {
    ...
    $patterns = array(
        ...
        '#^https://license\.techon\.lk$#i',
        '#^https://([a-z0-9-]+\.)*erp\.techon\.lk(:\d+)?$#i',
    );
...
function techon_license_apply_cors_headers($allowHeaders) {
    header('Content-Type: application/json; charset=utf-8');
    $origin = isset($_SERVER['HTTP_ORIGIN']) ? trim((string) $_SERVER['HTTP_ORIGIN']) : '';
    if ($origin !== '' && techon_license_cors_origin_allowed($origin)) {
